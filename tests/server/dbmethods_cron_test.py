...
class dbmethods_cron_test(dbmethods_base):
    def test_600_cron_dataset_completion(self):
        ...
        try:
            ...
            logger.error('Error running dbmethods cron_dataset_completion test - %s',str(e))
            printer('Test dbmethods cron_dataset_completion',False)
            raise
        else:
            printer('Test dbmethods cron_dataset_completion')
