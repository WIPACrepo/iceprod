...
logger = logging.getLogger('GtkModuleList')
...
class GtkModuleList:
    ...
    def __init__(self,iconfig,pdb,gtkmod,title="Available Modules"):
    ...
    def display(self):
        ...
        for mp in self.iconfig.GetMetaProjectList():
            for p in self.pdb.GetProjects(mp.GetId()):
            	...
            	modules = self.pdb.GetModulesP(pname,pver)
            	for m in modules:
            		if not self.iconfig.GetModule(m.GetName()):
            			row = [m.GetId(),m.GetName(), m.GetClass(),pname,False]  
    ...
    def add_module(self, tv, path, viewcol):
		...
		if not proj: 
			for mp in self.iconfig.GetMetaProjectList():
				for p in self.pdb.GetProjectsMM(newmod,mp):
					mp.AddProject(p.GetName(),p)
					if p.GetName() == proj_name: proj = p
		if not proj: 
			raise Exception, "Could not find project for '%s'"% newmod.GetName()
		newmod.AddProject(proj_name,proj)
