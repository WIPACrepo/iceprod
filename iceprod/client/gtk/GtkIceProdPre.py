...
#
"""
  Gtk from for displaying and configuring IceProdPre Modules in 
...
from GtkIceProdPreList import GtkIceProdPreList 
from iceprod.core.dataclasses import *
from GtkIPModule import *
import logging

logger = logging.getLogger('GtkIceProdPre')

class GtkIceProdPre(GtkIPModule):
    ...
    def add_module(self, b):
		self.mlist = GtkIceProdPreList(self.iconfig,self.pdb,self)
    ...
    def delete_module(self, b):
		...
		self.iconfig.RemoveIceProdPre(name)
		self.showmodules()
    ...
    def configure_module(self, tv, path, viewcol):
		...
		module = self.iconfig.GetIceProdPre(name)
		GtkParameterList(module,self.paramwidth,self.paramheight)
    ...
    def col_edited( self, cell, path, new_text, model ):
        ...
        row = model[path]
        mname = row[0]
        mod = self.iconfig.GetIceProdPre(mname)
        if mod:
			self.iconfig.RemoveIceProdPre(mod.GetName())
			mod.SetName(u'%s' % new_text)
			self.iconfig.AddIceProdPre(mod)
			row[0] = u'%s' % new_text

    def __init__(self,iconfig,pdb,paramwidth=750,paramheight=600):
        self.iconfig = iconfig
        self.pdb = pdb
        self.paramwidth = paramwidth
        self.paramheight = paramheight
        ...
        self.b0 = gtk.Button('Add IceProdPre',stock=gtk.STOCK_ADD)
        self.b0.connect('clicked', self.add_module)
        self.b1 = gtk.Button('Delete IceProdPre',stock=gtk.STOCK_DELETE)
...
# -------
    ...
    def drag_data_received_data(self, treeview, context, x, y, selection,
        ...
        if drop_info:
            ...
            row = []
            module = self.iconfig.RemoveIceProdPre(data)
            row.append(module.GetName())
            row.append(module.GetClass())
            if (position == gtk.TREE_VIEW_DROP_BEFORE
                or position == gtk.TREE_VIEW_DROP_INTO_OR_BEFORE):
                model.insert_before(iter, row)
                self.iconfig.InsertIceProdPre(path[0],module)
            else:
                model.insert_after(iter, row)
                self.iconfig.InsertIceProdPre(path[0]+1,module)
    ...
    def showmodules(self):
	  ...
	  modules = self.iconfig.GetIceProdPres()
	  for m in modules: 
	  	row = []
